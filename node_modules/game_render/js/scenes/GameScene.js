...
import MineManager from '../utils/MineManager.js';
...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        let map_data = constant.MAP_DATA.split('\n');
    ...
    runGameTurn(dataGame) {
        this.player_1.setData(this, dataGame.player_1);
        this.player_2.setData(this, dataGame.player_2);

        this.bombsManger.setBomb(this, dataGame.bombs);

        this.brickManager.setBrick(this, dataGame.bricks);

        this.mineManager.setData(this, dataGame.mine);
        ...
        dataGame.explosions.forEach(function (item) {
            ...
            for (let i = 1; i <= item.d; i++)
                this.add.existing(new Explosion(this, item.x, item.y + i, (i === item.d) ? 'explosion_down' : 'explosion_vertical'));
            for (let i = 1; i <= item.u; i++)
                this.add.existing(new Explosion(this, item.x, item.y - i, (i === item.u) ? 'explosion_up' : 'explosion_vertical'));
            for (let i = 1; i <= item.l; i++)
                this.add.existing(new Explosion(this, item.x - i, item.y, (i === item.l) ? 'explosion_left' : 'explosion_horizontal'));
            for (let i = 1; i <= item.r; i++)
                this.add.existing(new Explosion(this, item.x + i, item.y, (i === item.r) ? 'explosion_right' : 'explosion_horizontal'));
        }.bind(this));
    }
}
